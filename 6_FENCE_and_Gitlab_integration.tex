...
\IBnote{}{Aren't these the old names?}\Maurizionote{}{not sure which names we want to use. t.b.d.}
...
\GSnote{}{Rephrase as: a way in which we protect against user mistakes are through two common errors: ...}\Maurizionote{}{tbd: there's no user mistake. It's a control we have to run to make sure we are using the correct method when pushing to Gitlab}Another interaction is needed to avoid two types of errors that will result in a blocker operation:
