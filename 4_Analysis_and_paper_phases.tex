...
\begin{figure}[htb]
  ...
  \GSnote[inline]{}{Expand this caption please. Mention what the reference code is, the name of the analysis, and what some of the data provided is.}}%
...
It also has the actions that can be triggered from one step with their function callback and the next state which the workflow should be moved to. An example is shown in \cref{app:analysis:workflow}.
...
\begin{figure}[htb]
  ...
  \caption{Summary of the \Class{Messenger} class infrastructure.
    \GSnote[inline]{}{Use this caption to explain the entire workflow depicte d in the diagram. Where am I looking at first? What's the order in which things happen? What's the computer icon in the middle? A user?}}%
...
These are used to check user \GSnote{clearance}{authorization}:
...
\GSnote{An example is shown in \cref{app:analysis:useredit}}{Why can't we show the example here near the explanation?}.
