...
Each of those metadata should be filled in a \GSnote{specific order}{Is the order documented in this note?},
\GSnote{by certain users}{Is that documented in this note? Who?} and trigger automatic emails or egroups~\cite{egroups} updates along the process.
...
Whenever a specific new object needs to be created, a group of three files are needed: the Factory, the Builder and the Model files. The below diagram illustrates how these classes communicate:
\begin{figure}[htb]
    \centering
    \includegraphics[width=0.9\textwidth]{builders_and_factories.png}
